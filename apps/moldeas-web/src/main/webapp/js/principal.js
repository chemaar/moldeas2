...
function codeList(){
	$.ajax( {
	type:'GET',
	url:'services/moldeas/psc/listCodes',
	dataType: "xml",
...
function nutsCodeList(){
	$.ajax( {
	type:'GET',
	url:'services/moldeas/psc/listNuts',
	dataType: "xml",
	contentType: "application/xml",
...
function search(){
	...
	if ($('#cpvCodes option').size() != 0 ){			
		...
		var minCost = $("#slider-range").slider("values", 0);
		var maxCost = $("#slider-range").slider("values", 1);
		var minDuration = $("#slider-range-duration").slider("values", 0);
		var maxDuration = $("#slider-range-duration").slider("values", 1);
		var typeEnterprise = $('input[@name="company"]:checked').val(); //FIXME: get the value of first radio group
		$.ajax( {
		type:'GET',
		url:'services/moldeas/search?cpvCodes='+cpvCodes+'&nutsCodes='+nutsCodes+'&minYear='+minYear+'&maxYear='+maxYear+'&minCost='+minCost+'&maxCost='+maxCost+'&minDuration='+minDuration+'&maxDuration='+maxDuration+'&typeEnterprise='+typeEnterprise,
		dataType: "xml",
		contentType: "application/xml",
		success: GetSearchOk
		});
