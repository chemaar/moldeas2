...
En el caso objeto de estudio, el punto de partida son conjuntos de datos generados por las Administraciones Públicas y procesados por una empresa, por lo que parcialmente la fuente de los mismos se considera 
...
de una PSC y el CPV 2008. La intersección se realiza mediante el comando: 

\texttt{awk 'NR==FNR{a[\$0];next} \$0 in a' psc-5 cpv-2008-6  >  psc-5-6}
...
datos de la PSC y genera un listado de conceptos pertenecientes a la PSC, fichero \texttt{psc-po-cpv}, que tienen relación 
...
\item Adición de todos los valores recogidos para la creación de porcentajes mediante una hoja de cálculo.
\end{enumerate}

 \item Determinación del tamaño muestral. Ya se ha señalado en los aspectos cuantitativos del experimento
...
A continuación se realiza una validación pormenorizada para cada uno de los conjuntos de datos y enfoques 
...
el contraste respecto a las actuales versiones en \gls{HTML}, \gls{PDF}, MSExcel, etc., y sistemas de recuperación opacos suministradores que generan servicios de negocio sobre ellos, es crítico, ya que simplemente el manejo de información 
...
resaltar que el enfoque realizado en este documento es factible debido a la posibilidad de utilizar \opendata y mejorar 
con la iniciativa de \linkeddata. Adicionalmente, se establece una fórmula para el cálculo de la ganancia al introducir 
...
para la realización de aplicaciones de carácter crítico. En este sentido y siguiendo con la validación realizada 
se obtiene un incremento, al menos, entre el $6.64\%$ (con \textit{ProductOntology}) y $8.65\%$ exacto del vocabulario controlado 
de entrada. En el caso particular de recuperación de documentos estos porcentajes son muy relevantes pero incluso, es conveniente 
la introducción de términos ``relacionados'' bajo un umbral de incertidumbre ya que el funcionamiento 
